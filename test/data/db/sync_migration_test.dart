...
NativeDatabase _seedV14Database() {
  final raw = sqlite3.sqlite3.openInMemory();
  raw.execute('''
    CREATE TABLE deck_table (
      id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,
  ...
  ''');
  raw.execute('''
    CREATE TABLE analytics_event_table (
...
void main() {
  ...
  test('v14 -> v15 migration adds remoteLastActivityAt to deck_table',
      () async {
    final db = AppDatabase.withExecutor(_seedV14Database());
    addTearDown(db.close);

    final decks = await db.select(db.deckTable).get();
    expect(decks, hasLength(1));
    // Pre-existing rows backfill to null; only a successful card pull
    // populates it going forward — same convention as remoteUpdatedAt.
    expect(decks.single.remoteLastActivityAt, isNull);

    // The new column is queryable/writable post-migration, independently of
    // the pre-existing remoteUpdatedAt column.
    final now = DateTime.fromMillisecondsSinceEpoch(
        (DateTime.now().millisecondsSinceEpoch ~/ 1000) * 1000);
    await (db.update(db.deckTable)..where((t) => t.id.isValue(1))).write(
      DeckTableCompanion(remoteLastActivityAt: Value(now)),
    );
    final updated = await (db.select(db.deckTable)
          ..where((t) => t.id.isValue(1)))
        .getSingle();
    expect(updated.remoteLastActivityAt, now);
    expect(updated.remoteUpdatedAt, isNull);
  });
}
